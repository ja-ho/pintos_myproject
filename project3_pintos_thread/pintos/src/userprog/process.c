...
start_process (void *file_name_)
{
  ...
  success = load (file_name, &if_.eip, &if_.esp, &save_ptr);
	tempThread = thread_current();
	if(!success) {
		tempThread->childProcess->load = 2;
	} else {
		tempThread->childProcess->load = 1;
	}
...
process_wait (tid_t child_tid UNUSED) 
{
	...
	if (childProcess->wait == true) {
		return -1;
	} else {
		childProcess->wait = true;
		while(childProcess->exit != true) {
			barrier();
		}
		status = childProcess->exit_status;
...
process_exit (void)
{
  ...
  struct list_elem *e;
	uint32_t *pd;
	//something is weird
	//close all files which is opened by this proces
	for(e = list_begin(&cur->file_list); e != list_end(&cur->file_list); e = list_next(e)) {
		process_file *tempFile = list_entry(e, process_file, elem);
		file_allow_write(tempFile->file);
		file_close(tempFile->file);
		list_remove(&tempFile->elem);
		free(tempFile);
	}

	//free child list
	for(e = list_begin(&cur->child_list); e != list_end(&cur->child_list); e = list_next(e)) {
		child_process *childProcess = list_entry(e, child_process, elem);
		list_remove(&childProcess->elem);
		free(childProcess);
	}	
	...
	/*for(e=list_begin(&all_list); e!= list_end(&all_list); e= list_next(e)) {
		...
		if(temp->tid == cur->parent_tid) {
			alive = true;
		}
	}*/
	//set exit value to true in case killed by the kernel
	if(isThreadAlive(cur->parent_tid)) {
		cur->childProcess->exit = true;
	}



	/* Destroy the current process's page directory and switch back
     to the kernel-only pagef directory. */
  pd = cur->pagedir;
  if (pd != NULL) 
    {
...
setup_stack (void **esp, const char *file_name, char **save_ptr) 
{
 ...
 /* argv = (char *)malloc(allocateSize*sizeof(char *));	
	for(token = (char *)file_name; token != NULL;
		...
		*esp -= strlen(token) + 1;
		...
		if(argc >= allocateSize) {
			allocateSize *= 2;
			argv = realloc(argv, allocateSize*sizeof(char *));
		}
	}*/
	
	argv[argc]=0;
	//align to word size (4 bytes)i
